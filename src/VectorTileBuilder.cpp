...
void flightsimlib::cgl::VectorTileBuilder::AddRoad(
    ERoadType type, RoadFeature& road, const std::vector<VectorVertex>& vertices)
{
...
void flightsimlib::cgl::VectorTileBuilder::AddLand(
    ELandType type, LandFeature& land, const std::vector<VectorVertex>& vertices)
{
...
void flightsimlib::cgl::VectorTileBuilder::AddWater(
    EWaterType type, std::vector<std::vector<VectorVertex>>& vertices, const std::vector<float>& heights)
{
    ...
    for (const auto& verts : vertices)
    {
        water.push_back(WaterPolygon{
            heights[count++], static_cast<uint16_t>(m_vertices.size()), static_cast<uint16_t>(verts.size())});

        m_vertices.insert(verts.cbegin(), verts.cbegin(), verts.cend());
    }
}

void flightsimlib::cgl::VectorTileBuilder::AddRiver(RiverFeature river, const std::vector<VectorVertex>& vertices)
{
...
void flightsimlib::cgl::VectorTileBuilder::AddPoints(EPointType type, const std::vector<VectorVertex>& vertices)
{
    ...
    m_points[type].emplace_back(
        VectorVertex{static_cast<uint16_t>(m_vertices.size()), static_cast<uint16_t>(vertices.size())});
}

void flightsimlib::cgl::VectorTileBuilder::AddRail(
    ERailType type, RailFeature& rail, const std::vector<VectorVertex>& vertices)
{
...
void flightsimlib::cgl::VectorTileBuilder::AddPower(
    EPowerType type, PowerFeature& power, const std::vector<VectorVertex>& vertices)
{
...
void flightsimlib::cgl::VectorTileBuilder::AddUnknown1(
    EUnknown1Type type, Unknown1Feature& unknown1, const std::vector<VectorVertex>& vertices)
{
...
void flightsimlib::cgl::VectorTileBuilder::AddUnknown2(
    EUnknown2Type type, Unknown2Feature& unknown2, const std::vector<VectorVertex>& vertices)
{
...
flightsimlib::cgl::IVectorTile* flightsimlib::cgl::VectorTileBuilder::Build()
{
    ...
    for (auto i = 0; i < IVectorTile::RoadTypeCount; ++i)
    {
        ...
        for (auto& feature : road->second)
        {
            ...
            tile->RoadVertices.insert(m_vertices.cbegin() + feature.Start, m_vertices.cbegin() + feature.Start,
    ...
    for (auto i = 0; i < IVectorTile::LandTypeCount; ++i)
    {
        ...
        for (auto& feature : land->second)
        {
            ...
            tile->LandVertices.insert(m_vertices.cbegin() + feature.Start, m_vertices.cbegin() + feature.Start,
                m_vertices.cbegin() + feature.Start + feature.End);
    ...
    for (auto i = 0; i < static_cast<int>(m_water_polygons.size()); ++i)
    {
        ...
        tile->WaterFeatures.emplace_back(
            WaterFeature{static_cast<uint16_t>(type), static_cast<uint16_t>(polygons.size())});

        for (auto& polygon : polygons)
        {
            const auto index = static_cast<uint16_t>(tile->WaterVertices.size());

            tile->WaterVertices.insert(m_vertices.cbegin() + polygon.Start, m_vertices.cbegin() + polygon.Start,
    ...
    for (auto& river : m_rivers)
    {
        ...
        tile->RiverVertices.insert(m_vertices.cbegin() + river.Start, m_vertices.cbegin() + river.Start,
    ...
    for (auto i = 0; i < IVectorTile::PointTypeCount; ++i)
    {
        ...
        {
            tile->PointVertices.insert(m_vertices.cbegin() + feature.Start, m_vertices.cbegin() + feature.Start,
                m_vertices.cbegin() + feature.Start + feature.End);
    ...
    for (auto i = 0; i < IVectorTile::RailTypeCount; ++i)
    {
        ...
        for (auto& feature : rail->second)
        {
            ...
            tile->RailVertices.insert(m_vertices.cbegin() + feature.Start, m_vertices.cbegin() + feature.Start,
                m_vertices.cbegin() + feature.Start + feature.End);
    ...
    for (auto i = 0; i < IVectorTile::PowerTypeCount; ++i)
    {
        ...
        for (auto& feature : power->second)
        {
            ...
            tile->PowerVertices.insert(m_vertices.cbegin() + feature.Start, m_vertices.cbegin() + feature.Start,
                m_vertices.cbegin() + feature.Start + feature.End);
